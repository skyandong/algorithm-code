...
- **TF 饱和速度固定**：Lucene 经典 TF-IDF 的 TF 项是 √freq，出现 100 次的得分是出现 1 次的 10 倍（不是 100 倍），但饱和曲线固定、不可调
...
| 改进点 | 效果 |
|--------|------|
| TF 饱和且可调 | k1 控制饱和速度，比 √freq 更平缓，避免高频词碾压 |
| 字段长度归一化 | 短文档中词的权重更高，长文档不占便宜 |
| IDF 公式不同 | 定性上仍是词越稀有权重越高，但公式改为 ln(1+(N-n+0.5)/(n+0.5))，不同于经典 TF-IDF 的 ln(N/n) |
