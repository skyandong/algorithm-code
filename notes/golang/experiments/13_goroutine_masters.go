...
// mastersBroadcastStop 笔记 13 第 1.2 节：close(finish) 一次广播，N 个 goroutine 同时收到。
...
// mastersWaitMany 笔记 13 第 1.2 节：nil channel 禁用已关闭分支，避免死循环。
...
// mastersGOMAXPROCS1 笔记 13 第 2.1 节：单核上并发执行但非并行。
...
// mastersGoschedSay 笔记 13 第 4.1 节：Gosched 主动让出时间片，实现交替输出（文档注明 GOMAXPROCS=1 时）。
...
// mastersChannelSum 笔记 13 第 4.1 节：两个 goroutine 各算一半，channel 汇总。
...
// mastersFibonacci 笔记 13 第 4.1 节：生产者 close，range 自动退出。
...
// mastersPipeline 笔记 13 第 6.1 节：gen → sq → sq 三阶段流水线。
...
// mastersSelectTimeout 笔记 13 第 4.1 节：select + 超时退出模式。
