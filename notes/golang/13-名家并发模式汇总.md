...
> 注：缓冲区中未读出的数据会先被接收完，之后才返回零值（见下方实战代码输出 `1 2 3 0`），详见 `06-Channel内部与nil语义.md`。
...
已在 `notes/golang/12-Goroutine面试题集.md` 中收录，这里补充一道额外的：
...
| **面试** | 20 道并发题目（编程+选择+简答） | 鸟窝: Go 并发编程小测验, interview-go → `12-Goroutine面试题集.md` |
...
对应代码：[experiments/13_goroutine_masters.go](experiments/13_goroutine_masters.go)
