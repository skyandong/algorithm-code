...
>
> 本文基于仓库 `go.mod` 声明的 Go 1.26 编写，所有行为按当前版本描述。
>
...
`for` 循环变量按迭代创建，闭包各自捕获自己的变量；这里仍然显式计算 `ip` 并作为参数传入，使生命周期和意图更清晰。
...
正数 `Add` 应该在对应的 `Wait` 开始前完成。`WaitGroup.Go` 能减少常见的 `Add`/`Done` 配对错误，但不能让错误的任务生命周期变正确。
