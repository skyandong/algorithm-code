...
> 注：缓冲区中未读出的数据会先被接收完，之后才返回零值（见下方实战代码输出 `1 2 3 0`），详见 `05-Channel内部与nil语义.md`。
...
已在 `notes/golang/11-Goroutine面试题集.md` 中收录，这里补充一道额外的：
...
| **面试** | 20 道并发题目（编程+选择+简答） | 鸟窝: Go 并发编程小测验, interview-go → `11-Goroutine面试题集.md` |
...
>
> ```go
> // ⚠️ 循环中使用 time.After —— 每次迭代都创建新 timer，只剩分配开销（不可达后可被 GC 回收，不再泄漏）
...
> ```
>
> 当前 timer 语义（`time/sleep.go` 文档明确说明）：不可达的 timer 即使未到期、未 Stop 也可以被 GC 回收，因此循环中的 `time.After` **不构成内存泄漏**，只剩每次迭代的分配开销；timer channel 是同步发送，`Reset`/`Stop` 不要求先排空。追求零分配时用 `NewTimer` + `Reset`。

---

## 实验

对应代码：[experiments/13_goroutine_masters.go](experiments/13_goroutine_masters.go)、[13_goroutine_masters_test.go](experiments/13_goroutine_masters_test.go)（断言式单测，无打印）
